...
if df is not None:
    if st.session_state.view_mode in ["match_detail", "h2h_detail"]:
            ...
            if sel_ref != "Välj domare...":
                ...
                if not r_df.empty:
                    ...
                    for idx_r, row_r in r_df_sorted.iterrows():
                        h_c, a_c = row_r['Gula kort Hemma'], row_r['Gula Kort Borta']
                        st.markdown(f"""
                            <div class="match-row">
                                ...
                                <div style="margin:0 15px; background:#f0f0f0; padding:2px 10px; border-radius:4px; display:flex; gap:10px;">
                                    <span style="font-weight:bold;">{int(h_c)}</span> - <span style="font-weight:bold;">{int(a_c)}</span>
                                </div>
        ...
        with tab5:
            ...
            c1, c2 = st.columns(2)
            with c1: num_matches = st.slider("Antal matcher för analys:", 1, 20, 10)
            with c2: 
                all_leagues = ["Alla"] + sorted(df['response.league.name'].unique().tolist())
                sel_league = st.selectbox("Välj liga:", all_leagues, key="top_league_filter")
            
            filtered_df = df[df['response.fixture.status.short'] == 'FT']
            if sel_league != "Alla": filtered_df = filtered_df[filtered_df['response.league.name'] == sel_league]

            if top_cat == "Lag":
                team_stats = []
                teams = sorted(pd.concat([filtered_df['response.teams.home.name'], filtered_df['response.teams.away.name']]).unique())
                for t in teams:
                    avg = get_rolling_card_avg(t, df, n=num_matches)
                    team_stats.append({'Lag': t, 'Snitt Kort': round(avg, 2)})
                ...
                upcoming = upcoming[upcoming['datetime'] >= now_check].head(20)
                hot_list = []
                for _, row in upcoming.iterrows():
                    h_avg = get_rolling_card_avg(row['response.teams.home.name'], df, n=num_matches)
                    a_avg = get_rolling_card_avg(row['response.teams.away.name'], df, n=num_matches)
                    total_power = h_avg + a_avg
                    hot_list.append({'Match': f"{row['response.teams.home.name']} vs {row['response.teams.away.name']}", 'Total Kortstyrka': round(total_power, 2), 'Hemmasnitt': round(h_avg, 2), 'Bortasnitt': round(a_avg, 2)})
                st.dataframe(pd.DataFrame(hot_list).sort_values('Total Kortstyrka', ascending=False), use_container_width=True, hide_index=True)
