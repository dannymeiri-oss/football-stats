...
# --- 1. KONFIGURATION (PERFECT LAYOUT - DEFINITIV FIX) ---
...
st.markdown("<p class='sub-title'>Perfect Layout - Centered & Fixed</p>", unsafe_allow_html=True)
    ...
    if st.session_state.view_mode == "match_detail" or st.session_state.view_mode == "h2h_detail":
        ...
        st.markdown(f"""
            <div class='centered-header'>
                <img src='{m['response.teams.home.logo']}' width='80'>
                <h1 style='margin:0;'>{h_team} vs {a_team}</h1>
                <img src='{m['response.teams.away.logo']}' width='80'>
            </div>
        """, unsafe_allow_html=True)
