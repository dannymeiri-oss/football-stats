...
GID_RAW = "0"
GID_STANDINGS = "DITT_GID_HÄR" # Glöm inte att fylla i GID för din tabell-flik!
...
def get_match_odds(home_team, away_team):
    ...
    # Hämtar 1X2 och Över/Under 2.5 från Unibet (region EU)
    ...
    try:
        ...
        for match in res:
            # Enkel sökning: Ser om lagnamnet från Google Sheets finns i API-svaret
                ...
                if 'bookmakers' in match and len(match['bookmakers']) > 0:
                    for market in match['bookmakers'][0]['markets']:
                        ...
                        if market['key'] == 'totals':
                            totals_odds = [o for o in market['outcomes'] if o['point'] == 2.5]
                
                return h2h_odds, totals_odds
        return None, None
    except:
        return None, None
...
if df is not None:
    ...
    # VY 1: STATISTIK (SPELADE MATCHER)
    ...
    # VY 2: H2H MED FÖRVÄNTAN & ODDS (KOMMANDE MATCHER)
        ...
        with main_c2:
            if not h_stats.empty and not a_stats.empty:
                ...
                st.divider()
                st.markdown("<h3 style='text-align: center;'>💸 Aktuella Marknadodds (Unibet)</h3>", unsafe_allow_html=True)
                h2h, totals = get_match_odds(h_team, a_team)
                if h2h or totals:
                    oc1, oc2 = st.columns(2)
                    with oc1:
                        st.write("**Matchvinnare (1X2)**")
                        if h2h:
                            for o in h2h: st.write(f"{o['name']}: **{o['price']}**")
                    with oc2:
                        st.write("**Över/Under 2.5 Mål**")
                        if totals:
                            for o in totals: st.write(f"{o['name']} 2.5: **{o['price']}**")
                else:
