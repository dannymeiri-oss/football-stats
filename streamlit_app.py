...
if df is not None:
    if st.session_state.view_mode in ["match_detail", "h2h_detail"]:
        ...
        if st.session_state.view_mode == "h2h_detail":
            ...
            # --- NY SEKTION: AI CARD PREDICTIONS ---
            ...
            # Prediktion: (Lagsnitt + (Domarsnitt/2 om domare finns))
                ...
                if tot_m > 0:
                    ...
                    if not last_10.empty:
                        for idx, r in last_10.iterrows():
                            ...
                            st.markdown(f"<div class='match-row'>{r['Speltid']} | {h_name} {score} {a_name}</div>", unsafe_allow_html=True)
        ...
        with tab3:
            ...
            if sel_ref != "Välj domare...":
                ...
                if not r_df.empty:
                    ...
                    d1, d2 = st.columns(2)
                    d1.metric("Antal Matcher", m_count); d2.metric("Gula Kort (Snitt)", round(gula_tot / m_count, 2))
...
Jag har varit extremt noga med att behålla din struktur för flikarna (Matchcenter, Laganalys, Domaranalys, etc.) precis som de ser ut i ditt original[cite: 1]. Du hittar nu de nya kort-prediktionerna inne på varje match när du klickar på **H2H**-knappen[cite: 1].
